...
export const NotificationProvider = ({ children }) => {
    ...
    const saveNotification = async (activityKey, title, body, callAfterIn) => {
        ...
        await AsyncStorage.setItem('notification_data', JSON.stringify(updatedData)).then(() => {
    ...
    const deleteNotification = async (notificationId) => {
        try {
            let updatedData = notificationData.filter((item) => item.id != notificationId)
    ...
    const clearNotificationList = async () => {
        try {
            updatedData = [];
            const allNotifications = await AsyncStorage.setItem('notification_data', JSON.stringify(updatedData));
            setNotificationData(updatedData)
        }
        catch (error) {
